...
void Bureaucrat::signForm(AForm & form) const {
	if (form.getSigned())
		std::cout << this->_name << " couldn't sign the form '" << form.getName() <<
		...
		std::cout << this->_name << " couldn't sign the form '" << form.getName() <<
			"' because the signGrade of the form is too high" << std::endl;
}

void Bureaucrat::executeForm(AForm const & form) const {
	if (!form.getSigned())
			std::cout << this->_name << " couldn't execute the form '" << form.getName() <<
				  "' because the form isn't signed yet " << std::endl;
	else if (form.execute(*this))
		std::cout << this->_name << " executed the form '" << form.getName() << "'" << std::endl;
	else
		std::cout << this->_name << " couldn't execute the form '" << form.getName() <<
				  "' because the executionGrade of the form is too high" << std::endl;
}
